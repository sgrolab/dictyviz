...
def main(zarr_file=None, crop_id=None):
    ...
    output_file = parent_dir + '/calcSlicedOrthoMaxProjs_out.txt'
    with open(output_file, 'w') as f:
        ...
        if os.path.isdir(os.path.join(analysis_dir, 'sliced_max_projections' + crop_id, 'maxx')):
            print('Sliced max projections already calculated, skipping calculation.', file=f)
            return

        # calculate max projections
        create_root_store(parent_dir + '/analysis/sliced_max_projections' + crop_id)
        ...
        dv.calc_sliced_max_projections(root, sliced_max_projections_root, res_lvl=0)
