...
optical_output_folder="${parent_directory}/sliced_X_optical_flow_output"
...
if [ -f "$flow_file" ] && [ -f "$movie_file" ]; then
    ...
    if [ -z "$cropID" ]; then
        # No crop ID provided
        bsub -n 12 -W 08:00 python3 slicedOpticalFlow.py "${zarr_folder}" "${channel}"
    else
        # Crop ID provided
        bsub -n 12 -W 08:00 python3 slicedOpticalFlow.py "${zarr_folder}" "${channel}" "${cropID}"
    fi
fi 